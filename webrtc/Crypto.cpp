...
const uint8_t* Crypto::GetHmacSha1(const std::string& key, const uint8_t* data, size_t len)
{
    ...
    int ret = EVP_MAC_init(Crypto::hmacSha1Ctx, reinterpret_cast<const unsigned char*>(key.c_str()), key.length(), sha1);
    if (ret != 1)
    {
        LOG_ERROR("OpenSSL EVP_MAC_init() failed with key '{}'", key);
        return nullptr;
    }
    ...
    if (ret != 1)
    {
        LOG_ERROR("OpenSSL EVP_MAC_update() failed with key '{}' and data length {} bytes", key, len);
        return nullptr;
    }
    ...
    if (ret != 1)
    {
        LOG_ERROR("OpenSSL HMAC_Final() failed with key '{}' and data length {} bytes", key, len);
        return nullptr;
    }
    if (resultLen != SHA_DIGEST_LENGTH)
    {
        LOG_ERROR("OpenSSL HMAC_Final() resultLen is {} instead of 20", resultLen);
        return nullptr;
    }
