...
IceServer::IceServer(Listener* listener, const std::string& usernameFragment, const std::string& password) : listener(listener), usernameFragment(usernameFragment), password(password)
{
    // Notify the listener.
...
void IceServer::ProcessStunPacket(RTC::StunPacket* packet, RTC::TransportTuple* tuple)
{
    ...
    if (packet->GetMethod() != RTC::StunPacket::Method::BINDING)
    {
        ...
        {
            LOG_WARN("unknown method {} in STUN Request => 400", static_cast<unsigned int>(packet->GetMethod()));
        ...
        {
            LOG_WARN("ignoring STUN Indication or Response with unknown method {}", static_cast<unsigned int>(packet->GetMethod()));
        }
    ...
    if (!packet->HasFingerprint() && packet->GetClass() != RTC::StunPacket::Class::INDICATION)
    {
        if (packet->GetClass() == RTC::StunPacket::Class::REQUEST)
        {
            LOG_WARN("STUN Binding Request without FINGERPRINT => 400");
        ...
        {
            LOG_WARN("ignoring STUN Binding Response without FINGERPRINT");
        }
    ...
    switch (packet->GetClass())
    {
        case RTC::StunPacket::Class::REQUEST:
        {
            ...
            {
                LOG_WARN("mising required attributes in STUN Binding Request => 400");
            ...
            switch (packet->CheckAuthentication(this->usernameFragment, this->password))
            {
                case RTC::StunPacket::Authentication::OK:
                {
                    ...
                    {
                        LOG_DEBUG("new ICE credentials applied");
                ...
                case RTC::StunPacket::Authentication::UNAUTHORIZED:
                {
                    ...
                    {
                        LOG_DEBUG("using old ICE credentials");

                        break;
                    }

                    LOG_WARN("wrong authentication in STUN Binding Request => 401");
                ...
                {
                    LOG_WARN("cannot check authentication in STUN Binding Request => 400");
            ...
            {
                LOG_WARN("peer indicates ICE-CONTROLLED in STUN Binding Request => 487");
            ...
            LOG_DEBUG("processing STUN Binding Request [Priority:{}, UseCandidate:{}]", static_cast<uint32_t>(packet->GetPriority()), packet->HasUseCandidate() ? "true" : "false");
        ...
        {
            LOG_DEBUG("STUN Binding Indication processed");
        ...
        {
            LOG_DEBUG("STUN Binding Success Response processed");
        ...
        {
            LOG_DEBUG("STUN Binding Error Response processed");
...
void IceServer::ForceSelectedTuple(const RTC::TransportTuple* tuple)
{
    ...
    {
        LOG_ERROR("cannot force the selected tuple if there was not a selected tuple");
        return;
    }
    ...
    if (!storedTuple)
    {
        LOG_ERROR("cannot force the selected tuple if the given tuple was not already a valid tuple");
        return;
    }
...
void IceServer::HandleTuple(RTC::TransportTuple* tuple, bool hasUseCandidate, bool hasNomination, uint32_t nomination)
{
    switch (this->state)
    {
        case IceState::NEW:
        {
            ...
            {
                LOG_ERROR("state is 'new' but there are {} tuples", this->tuples.size());
                return;
            }
            ...
            if (this->selectedTuple)
            {
                LOG_ERROR("state is 'new' but there is selected tuple");
                return;
            }

            if (!hasUseCandidate && !hasNomination)
            {
                LOG_DEBUG("transition from state 'new' to 'connected' [hasUseCandidate:{}, hasNomination:{}, nomination:{}]",
            ...
            else
            {
                ...
                {
                    LOG_DEBUG("transition from state 'new' to 'completed' [hasUseCandidate:{}, hasNomination:{}, nomination:{}]",
                              hasUseCandidate ? "true" : "false",
                              hasNomination ? "true" : "false",
                              nomination);
        ...
        case IceState::DISCONNECTED:
        {
            ...
            if (!this->tuples.empty())
            {
                LOG_ERROR("state is 'disconnected' but there are {} tuples", this->tuples.size());
                return;
            }
            ...
            if (this->selectedTuple)
            {
                LOG_ERROR("state is 'disconnected' but there is selected tuple");
                return;
            }

            if (!hasUseCandidate && !hasNomination)
            {
                LOG_DEBUG("transition from state 'disconnected' to 'connected' [hasUseCandidate:{}, hasNomination:{}, nomination:{}]",
                          hasUseCandidate ? "true" : "false",
                          hasNomination ? "true" : "false",
                          nomination);
            ...
            else
            {
                ...
                if ((hasNomination && nomination > this->remoteNomination) || !hasNomination)
                {
                    LOG_DEBUG("transition from state 'disconnected' to 'completed' [hasUseCandidate:{}, hasNomination:{}, nomination:{}]",
                              hasUseCandidate ? "true" : "false",
                              hasNomination ? "true" : "false",
                              nomination);
        ...
        case IceState::CONNECTED:
        {
            ...
            if (this->tuples.empty())
            {
                LOG_ERROR("state is 'connected' but there are no tuples");
                return;
            }
            ...
            if (!this->selectedTuple)
            {
                LOG_ERROR("state is 'connected' but there is not selected tuple");
                return;
            }
            ...
            {
                LOG_DEBUG("transition from state 'connected' to 'completed' [hasUseCandidate:{}, hasNomination:{}, nomination:{}]",
                          hasUseCandidate ? "true" : "false",
                          hasNomination ? "true" : "false",
                          nomination);
        ...
        case IceState::COMPLETED:
        {
            ...
            if (this->tuples.empty())
            {
                LOG_ERROR("state is 'completed' but there are no tuples");
                return;
            }
            ...
            if (!this->selectedTuple)
            {
                LOG_ERROR("state is 'completed' but there is not selected tuple");
                return;
            }
...
inline RTC::TransportTuple* IceServer::AddTuple(RTC::TransportTuple* tuple)
{
    ...
    {
        LOG_WARN("too too many tuples, removing the oldest non selected one");
        ...
        if (!removedTuple)
        {
            LOG_ERROR("couldn't find any tuple to be removed");
            return nullptr;
        }
