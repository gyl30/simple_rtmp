...
StunPacket* StunPacket::Parse(const uint8_t* data, size_t len)
{
    ...
    // Get STUN method.
    const uint16_t msgMethod = (msgType & 0x000f) | ((msgType & 0x00e0) >> 1) | ((msgType & 0x3E00) >> 2);
    ...
    auto* packet = new StunPacket(static_cast<Class>(msgClass), static_cast<Method>(msgMethod), data + 8, data, len);
    ...
    while (pos + 4 <= len)
    {
        ...
        {
            LOG_WARN("the attribute length exceeds the remaining size, packet discarded");
        ...
        if (hasFingerprint)
        {
            LOG_WARN("attribute after FINGERPRINT is not allowed, packet discarded");
        ...
        switch (attrType)
        {
            ...
            case Attribute::PRIORITY:
            {
                ...
                if (attrLength != 4)
                {
                    LOG_WARN("attribute PRIORITY must be 4 bytes length, packet discarded");
            ...
            case Attribute::ICE_CONTROLLING:
            {
                ...
                if (attrLength != 8)
                {
                    LOG_WARN("attribute ICE-CONTROLLING must be 8 bytes length, packet discarded");
            ...
            case Attribute::ICE_CONTROLLED:
            {
                ...
                if (attrLength != 8)
                {
                    LOG_WARN("attribute ICE-CONTROLLED must be 8 bytes length, packet discarded");
            ...
            case Attribute::USE_CANDIDATE:
            {
                ...
                if (attrLength != 0)
                {
                    LOG_WARN("attribute USE-CANDIDATE must be 0 bytes length, packet discarded");
            ...
            case Attribute::NOMINATION:
            {
                ...
                if (attrLength != 4)
                {
                    LOG_WARN("attribute NOMINATION must be 4 bytes length, packet discarded");
            ...
            case Attribute::MESSAGE_INTEGRITY:
            {
                ...
                if (attrLength != 20)
                {
                    LOG_WARN("attribute MESSAGE-INTEGRITY must be 20 bytes length, packet discarded");
            ...
            case Attribute::FINGERPRINT:
            {
                ...
                if (attrLength != 4)
                {
                    LOG_WARN("attribute FINGERPRINT must be 4 bytes length, packet discarded");
            ...
            case Attribute::ERROR_CODE:
            {
                ...
                if (attrLength < 4)
                {
                    LOG_WARN("attribute ERROR-CODE must be >= 4bytes length, packet discarded");
    ...
    if (pos != len)
    {
        LOG_WARN("computed packet size does not match total size, packet discarded");
    ...
    if (hasFingerprint)
    {
        ...
        const uint32_t computedFingerprint = Utils::Crypto::GetCRC32(data, fingerprintAttrPos) ^ 0x5354554e;
...
StunPacket::StunPacket(Class klass, Method method, const uint8_t* transactionId, const uint8_t* data, size_t size)
    : klass(klass), method(method), transactionId(transactionId), data(const_cast<uint8_t*>(data)), size(size)
{
}
...
StunPacket::Authentication StunPacket::CheckAuthentication(const std::string& localUsername, const std::string& localPassword)
{
    switch (this->klass)
    {
        case Class::REQUEST:
        case Class::INDICATION:
        {
            ...
            if (this->username.length() <= localUsernameLen || this->username.at(localUsernameLen) != ':' || (this->username.compare(0, localUsernameLen, localUsername) != 0))
            {
        ...
        {
            LOG_ERROR("cannot check authentication for a STUN response");
    ...
    const uint8_t* computedMessageIntegrity = Utils::Crypto::GetHmacSha1(localPassword, this->data, (this->messageIntegrity - 4) - this->data);
...
{
    if (this->klass != Class::REQUEST)
    {
        LOG_ERROR("attempt to create a success response for a non Request STUN packet");
        return nullptr;
    }
...
StunPacket* StunPacket::CreateErrorResponse(uint16_t errorCode)
{
    if (this->klass != Class::REQUEST)
    {
        LOG_ERROR("attempt to create an error response for a non Request STUN packet");
        return nullptr;
    }

    auto* response = new StunPacket(Class::ERROR_RESPONSE, this->method, this->transactionId, nullptr, 0);
...
void StunPacket::Authenticate(const std::string& password)
{
    ...
    {
        LOG_ERROR("cannot set password for ErrorResponse messages");
        return;
    }
...
void StunPacket::Serialize(uint8_t* buffer)
{
    ...
    bool addXorMappedAddress = ((this->xorMappedAddress != nullptr) && this->method == StunPacket::Method::BINDING && this->klass == Class::SUCCESS_RESPONSE);
    const bool addErrorCode = ((this->errorCode != 0u) && this->klass == Class::ERROR_RESPONSE);
    const bool addMessageIntegrity = (this->klass != Class::ERROR_RESPONSE && !this->password.empty());
    ...
    if (addXorMappedAddress)
    {
        switch (this->xorMappedAddress->sa_family)
        {
            ...
            default:
            {
                LOG_ERROR("invalid inet family in XOR-MAPPED-ADDRESS attribute");
    ...
    if (addXorMappedAddress)
    {
        ...
        switch (this->xorMappedAddress->sa_family)
        {
            case AF_INET:
            {
                ...
                attrValue[1] = 0x01;
                // Set port and XOR it.
                std::memcpy(attrValue + 2, &(reinterpret_cast<const sockaddr_in*>(this->xorMappedAddress))->sin_port, 2);
                attrValue[2] ^= StunPacket::magicCookie[0];
                attrValue[3] ^= StunPacket::magicCookie[1];
                // Set address and XOR it.
                std::memcpy(attrValue + 4, &(reinterpret_cast<const sockaddr_in*>(this->xorMappedAddress))->sin_addr.s_addr, 4);
            ...
            case AF_INET6:
            {
                ...
                attrValue[1] = 0x02;
                // Set port and XOR it.
                std::memcpy(attrValue + 2, &(reinterpret_cast<const sockaddr_in6*>(this->xorMappedAddress))->sin6_port, 2);
                attrValue[2] ^= StunPacket::magicCookie[0];
                attrValue[3] ^= StunPacket::magicCookie[1];
                // Set address and XOR it.
                std::memcpy(attrValue + 4, &(reinterpret_cast<const sockaddr_in6*>(this->xorMappedAddress))->sin6_addr.s6_addr, 16);
                attrValue[4] ^= StunPacket::magicCookie[0];
                attrValue[5] ^= StunPacket::magicCookie[1];
                attrValue[6] ^= StunPacket::magicCookie[2];
                attrValue[7] ^= StunPacket::magicCookie[3];
    ...
    if (addMessageIntegrity)
    {
        ...
        const uint8_t* computedMessageIntegrity = Utils::Crypto::GetHmacSha1(this->password, buffer, pos);
